...
for network in inventory:
    ...
    for station in network:
        ...
        code = ".".join([net, sta])
        print(f"Doing {code}")
        datapath = os.path.join("rf_data", code, "")
        ...
        try:
            ...
            stream.select(component=component, station=sta).sort(
                ["back_azimuth"]
            ).plot_rf(**kw)
            plt.savefig(
                f"{plotpath}/{code}_{component}_.1_1.pdf",
                bbox_inches="tight",
                pad_inches=0.2,
            )
            plt.close()

            print("No. of RF=", len(stream) // 3, "for station", code)
            print("--------------------------------------------------\n")
        except:
