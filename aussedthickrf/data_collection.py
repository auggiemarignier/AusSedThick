...
Modified from hiperseis/seismic/extract_event_traces.py
"""
...
def sw_catalog(catalog, min_mag):
    ...
    for i, e in enumerate(catalog):
        if e.magnitudes[0].mag < min_mag:
            continue
        if e.preferred_origin().depth / 1e3 > SW_MAX_DEPTH:
            continue
...
def extract_data(catalog, inventory, waveform_getter, event_trace_datafile,
    ...
    if rank == 0:
        ...
        for i in np.arange(nproc):
            nsl_dict.append(defaultdict(list))  # net.sta.loc -> cha
        ...
        for i in np.arange(nbogus):
            temp_dict['%i.%i.%i'%(i, i, i)] = '-1'
...
def main(inventory_file, network_list, station_list, waveform_database, event_catalog_file, event_trace_datafile,
    ...
    if not waveform_db_is_web:
        ...
        #################################################
        if (asdf_dataset.corrections_enabled()):
            pad = 3600
    else:
